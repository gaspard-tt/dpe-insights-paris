...
import {
  AlertTriangle, ArrowLeft, ArrowRight, BarChart3, CheckCircle, ExternalLink,
  Info, Lightbulb, TrendingDown, Wrench, PartyPopper, Flame, Droplets,
  Layers, Wind, RefreshCw, Target, PiggyBank, Euro, MapPin, Phone,
} from "lucide-react";
...
const Results = () => {
  ...
  const priorityConfig = {
    high: { bg: "bg-destructive/10", text: "text-destructive", border: "border-destructive/20", dot: "bg-destructive", accent: "border-l-destructive" },
    medium: { bg: "bg-warning/10", text: "text-warning", border: "border-warning/20", dot: "bg-warning", accent: "border-l-warning" },
    low: { bg: "bg-success/10", text: "text-success", border: "border-success/20", dot: "bg-success", accent: "border-l-success" },
  };
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto max-w-4xl px-4 py-8">
              ...
              <div className="grid gap-8 md:grid-cols-2">
                ...
                <div className="flex flex-col justify-center">
                  <div className="mb-4 text-center md:text-left">
                    <span className="text-sm font-medium text-muted-foreground">{t("results.class")}</span>
                  ...
                  <div className="mb-4 text-center md:text-left">
                    <span className="text-sm font-medium text-muted-foreground">{t("results.consumption")}</span>
                    <p className="text-3xl font-bold text-foreground">
                      {consumption} <span className="text-base font-normal text-muted-foreground">kWh/m²/an</span>
                    </p>
                  </div>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-2 mb-1.5">
                      ...
                      <span className={`flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-bold ${priorityConfig[w.severity].bg} ${priorityConfig[w.severity].text}`}>
        ...
        {/* ====== RECOMMENDATIONS - PROMINENT SECTION ====== */}
        <motion.section {...fadeInUp} transition={{ delay: 0.3 }} className="mb-10">
          <div className="overflow-hidden rounded-2xl border-2 border-primary/20 bg-card shadow-md">
            <div className="bg-primary/5 px-6 py-5 sm:px-8">
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                {t("results.recommendations.subtitle")}
              </p>
            </div>
            ...
            <div className="divide-y">
              {recommendations.map((rec, index) => (
                <motion.div
                  ...
                  className={`border-l-4 ${priorityConfig[rec.priority].accent} p-5 sm:p-6`}
                >
                  {/* Header */}
                  ...
                  {/* Cost & Aid - NEW prominent section */}
                  <div className="mt-3 grid gap-2 sm:grid-cols-2">
                    {rec.estimatedCost && (
                      <div className="flex items-start gap-2 rounded-lg bg-muted/40 px-3 py-2.5">
                        <Euro className="mt-0.5 h-4 w-4 shrink-0 text-amber" />
                        <div>
                          <span className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">{t("results.cost")}</span>
                          <p className="text-xs font-medium text-foreground">{rec.estimatedCost}</p>
                        </div>
                      </div>
                    )}
                    {rec.parisAid && (
                      <div className="flex items-start gap-2 rounded-lg bg-success/5 border border-success/10 px-3 py-2.5">
                        <PiggyBank className="mt-0.5 h-4 w-4 shrink-0 text-success" />
                        <div>
                          <span className="text-[10px] font-semibold uppercase tracking-wider text-success">{t("results.aid")}</span>
                          <p className="text-xs font-medium text-foreground">{rec.parisAid}</p>
                        </div>
                      </div>
                    )}
                  </div>
            ...
            <div className="space-y-5 p-6 sm:p-8">
              {[
                ...
                <div key={n} className="flex items-start gap-3">
                  ...
                  <div>
                    <h3 className="text-base font-semibold text-foreground">{t(`results.edu.${n}.title`)}</h3>
                    <p className="mt-1 text-sm leading-relaxed text-muted-foreground">{t(`results.edu.${n}.desc`)}</p>
                  </div>
                </div>
              ))}
            </div>
